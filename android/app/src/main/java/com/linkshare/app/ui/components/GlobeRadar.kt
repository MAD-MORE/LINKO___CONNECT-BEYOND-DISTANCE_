    ...
    Box(contentAlignment = Alignment.Center, modifier = Modifier.size(size)) {
        Canvas(Modifier.size(size)) {
            ...
            floatArrayOf(-0.72f, -0.42f, -0.18f, 0.18f, 0.42f, 0.72f).forEach { latitude ->
                ...
                val ry = radius * 0.13f
                drawOval(color.copy(alpha = if (abs(latitude) < 0.2f) 0.27f else 0.15f), topLeft = Offset(cx - rx, y - ry), size = Size(rx * 2f, ry * 2f), style = Stroke(gridStroke))
            }
            ...
            drawOval(color.copy(alpha = 0.30f), topLeft = Offset(cx - radius, cy - radius * 0.055f), size = Size(radius * 2f, radius * 0.11f), style = Stroke(1.2.dp.toPx()))
            ...
            if (flowing) {
                ...
                particles.forEachIndexed { index, particle ->
                    ...
                    if (inward) {
                        ...
                        drawCircle(color.copy(alpha = 0.62f), 1.3.dp.toPx(), Offset(sx, sy))
                        drawCircle(color.copy(alpha = 0.44f), 1.1.dp.toPx(), Offset(sx2, sy2))
                    }
                }
            } else if (!idle) {
