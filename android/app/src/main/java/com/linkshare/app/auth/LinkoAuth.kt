...
class LinkoAuth(context: Context) {
    ...
    fun signUp(email: String, password: String, displayName: String): AuthResult {
        ...
        return request("/auth/v1/signup", "POST", body, saveTokens = true).also { if (it.success) saveProfile(cleanName, null, null) }
    }
    ...
    fun updatePassword(password: String): AuthResult { if (password.length !in 8..72) return AuthResult(false, "password_length_invalid"); val token = currentAccessToken() ?: return AuthResult(false, "session_required", true); return request("/auth/v1/user", "PUT", JSONObject().put("password", password), token, false) }
