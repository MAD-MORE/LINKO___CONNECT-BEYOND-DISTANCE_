...
fun DiagnosticCenterScreen(results: List<DiagnosticResult>, onRunDiagnostics: () -> Unit, modifier: Modifier = Modifier) {
    ...
    Column(modifier.fillMaxSize().drawBehind {
        ...
        points.forEachIndexed { index, point -> drawCircle(networkColor.copy(alpha = 0.08f + 0.07f * pulse), if (index == 5) 14f else 9f, point); drawCircle(networkColor.copy(alpha = 0.20f * pulse), if (index == 5) 22f else 15f, point, style = Stroke(2f)) }
        ...
        Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) {
            ...
            Crossfade(running, label = "header-live") { active -> Icon(if (active) Icons.Default.Refresh else Icons.Default.CheckCircle, null, Modifier.size(30.dp)) }
        }
            ...
            Column(Modifier.fillMaxWidth().padding(16.dp)) {
                Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) {
                    Column(Modifier.weight(1f)) {
                        AnimatedContent(running to complete, label = "diagnostic-header") { (isRunning, isComplete) -> Text(if (isRunning) "System scan in progress" else if (isComplete) "System scan complete" else "System ready", style = MaterialTheme.typography.titleLarge) }
                ...
                Spacer(Modifier.height(12.dp)); LinearProgressIndicator({ progress }, Modifier.fillMaxWidth().height(7.dp)); Spacer(Modifier.height(8.dp))
        ...
        UpdateStatusCard(updateState, { updateManager.checkAndOfferUpdate() }, { updateManager.startUpdate() }, { updateManager.retry() })
        Spacer(Modifier.height(12.dp))
        ...
        Crossfade(running to complete, label = "diagnostic-summary") { (isRunning, isComplete) -> Text(if (isRunning) "○ CHECKING SYSTEM — $completedChecks/${DiagnosticCenterViewModel.TOTAL_CHECKS}" else if (isComplete && overall is DiagnosticOverallState.CONNECTED) "✓ DIAGNOSTICS PASSED — LINKO IS READY" else if (isComplete) "✕ DIAGNOSTICS COMPLETE — $failed CHECK(S) NEED ATTENTION" else "○ READY FOR SYSTEM CHECK", style = MaterialTheme.typography.titleMedium) }
        Spacer(Modifier.height(8.dp))
        Button(onClick = { onRunDiagnostics(); viewModel.runDiagnostics() }, enabled = !running, Modifier.fillMaxWidth()) { if (running) { CircularProgressIndicator(Modifier.size(20.dp), strokeWidth = 2.dp); Spacer(Modifier.size(10.dp)); Text("CHECKING…") } else Text(if (complete) "RUN AGAIN" else "RUN DIAGNOSTICS") }
    }
}
...
private fun UpdatePipelineCard(state: LinkoUpdateManager.UpdateState) {
    ...
    Card(Modifier.fillMaxWidth().animateContentSize()) { Column(Modifier.padding(14.dp)) {
        Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) { Text("UPDATE PIPELINE", style = MaterialTheme.typography.labelLarge); Text(if (stage == 3) "COMPLETE" else if (active) "IN PROGRESS" else "READY", style = MaterialTheme.typography.labelSmall) }
        Spacer(Modifier.height(10.dp))
        Row(Modifier.fillMaxWidth()) { listOf("Downloading", "Verifying", "Installing", "Updated").forEachIndexed { index, label -> Column(Modifier.weight(1f), horizontalAlignment = androidx.compose.ui.Alignment.CenterHorizontally) { Crossfade(index == stage, label = "pipeline-$index") { selected -> Icon(if (index == 3) Icons.Default.CheckCircle else if (selected) Icons.Default.Refresh else Icons.Default.SystemUpdate, label, Modifier.size(22.dp)) }; Spacer(Modifier.height(4.dp)); Text(label, style = MaterialTheme.typography.labelSmall) } } }
        AnimatedVisibility(active) { Column { Spacer(Modifier.height(8.dp)); if (state.status == LinkoUpdateManager.UpdateStatus.Downloading) { LinearProgressIndicator({ state.progressPercent / 100f }, Modifier.fillMaxWidth()); Spacer(Modifier.height(3.dp)); Text("${state.progressPercent}%", style = MaterialTheme.typography.labelSmall) } else LinearProgressIndicator(Modifier.fillMaxWidth()) } }
    } }
}
...
private fun UpdateStatusCard(state: LinkoUpdateManager.UpdateState, onCheck: () -> Unit, onUpdate: () -> Unit, onRetry: () -> Unit) {
    ...
    Card(Modifier.fillMaxWidth().animateContentSize()) { Column(Modifier.fillMaxWidth().padding(16.dp)) {
        Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) { Row(Modifier.weight(1f)) { Icon(Icons.Default.SystemUpdate, null, Modifier.size(22.dp)); Spacer(Modifier.size(8.dp)); Column { Text("UPDATE STATUS", style = MaterialTheme.typography.titleMedium); AnimatedContent(state.status, label = "update-stage") { status -> Text(when (status) { LinkoUpdateManager.UpdateStatus.Downloading -> "DOWNLOADING"; LinkoUpdateManager.UpdateStatus.Verifying -> "VERIFYING"; LinkoUpdateManager.UpdateStatus.Installing -> "INSTALLING"; LinkoUpdateManager.UpdateStatus.Installed -> "UPDATED"; else -> state.statusMessage.ifBlank { "UPDATE CHECK READY" } }, style = MaterialTheme.typography.labelSmall) } } }; IconButton(onCheck, enabled = !active) { Icon(Icons.Default.Refresh, "Check for updates") } }
        Spacer(Modifier.height(10.dp)); Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween) { Text("Current ${state.installedVersionName} (${state.installedVersionCode})", style = MaterialTheme.typography.bodySmall); Text("Latest ${state.latestVersionName ?: "—"} (${state.latestVersionCode ?: "—"})", style = MaterialTheme.typography.bodySmall) }
        AnimatedVisibility(state.status in setOf(LinkoUpdateManager.UpdateStatus.Downloading, LinkoUpdateManager.UpdateStatus.Verifying, LinkoUpdateManager.UpdateStatus.Installing)) { Column { Spacer(Modifier.height(10.dp)); if (state.status == LinkoUpdateManager.UpdateStatus.Downloading) { LinearProgressIndicator({ state.progressPercent / 100f }, Modifier.fillMaxWidth()); Text("${state.progressPercent}% downloaded", style = MaterialTheme.typography.labelSmall) } else LinearProgressIndicator(Modifier.fillMaxWidth()) } }
        AnimatedVisibility(state.errorMessage != null) { Row(Modifier.fillMaxWidth().padding(top = 10.dp)) { Icon(Icons.Default.Error, null, Modifier.size(18.dp)); Spacer(Modifier.size(6.dp)); Text(state.errorMessage.orEmpty(), style = MaterialTheme.typography.bodySmall) } }
        if (state.status == LinkoUpdateManager.UpdateStatus.UpdateAvailable) { Spacer(Modifier.height(10.dp)); Button(onUpdate, Modifier.fillMaxWidth()) { Icon(Icons.Default.Download, null); Spacer(Modifier.size(8.dp)); Text("UPDATE LINKO") } }
        if (state.status == LinkoUpdateManager.UpdateStatus.Error) { Spacer(Modifier.height(6.dp)); Button(onRetry, Modifier.fillMaxWidth()) { Text("RETRY UPDATE CHECK") } }
        if (state.status == LinkoUpdateManager.UpdateStatus.UpToDate) { Spacer(Modifier.height(6.dp)); Row(Modifier.fillMaxWidth(), Arrangement.Center) { Icon(Icons.Default.CheckCircle, null, Modifier.size(18.dp)); Spacer(Modifier.size(6.dp)); Text("LINKO IS UP TO DATE", style = MaterialTheme.typography.labelMedium) } }
    } }
}

@Composable
private fun DiagnosticRow(result: DiagnosticResult) { Card(Modifier.fillMaxWidth().animateContentSize()) { Row(Modifier.fillMaxWidth().padding(14.dp), Arrangement.SpaceBetween) { Column(Modifier.weight(1f)) { Text(result.name, style = MaterialTheme.typography.titleSmall); if (result.detail.isNotBlank()) Text(result.detail, style = MaterialTheme.typography.bodySmall); result.latencyMs?.let { Text("${it} ms", style = MaterialTheme.typography.labelSmall) } }; Crossfade(result.status, label = "diagnostic-icon-${result.name}") { status -> Icon(when (status) { DiagnosticStatus.PASS -> Icons.Default.CheckCircle; DiagnosticStatus.FAIL -> Icons.Default.Close; DiagnosticStatus.CHECKING -> Icons.Default.Warning; DiagnosticStatus.WAITING -> Icons.Default.Refresh; DiagnosticStatus.SKIPPED -> Icons.Default.Close }, status.name, Modifier.size(24.dp)) } } } }
