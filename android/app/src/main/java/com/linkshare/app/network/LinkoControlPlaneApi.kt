...
class LinkoControlPlaneApi(
    ...
    suspend fun sendFriendRequest(userId: String): Boolean = withContext(Dispatchers.IO) { request("POST", "/requests", JSONObject().put("receiverUserId", userId)); true }
    override suspend fun watchIncomingRequests(onRequest: (IncomingRequest) -> Unit) { getPendingProviderRequests().forEach { onRequest(it.toIncomingRequest()) } }
    ...
    suspend fun providerDeviceForUser(friendUserId: String): JSONObject = withContext(Dispatchers.IO) {
        ...
        request("GET", "/v1/providers/user/${java.net.URLEncoder.encode(clean, "UTF-8")}")
    }
    ...
    override suspend fun requestAccess(hostId: String): SignalingSession = withContext(Dispatchers.IO) {
        ...
        val session = request("POST", "/v1/sessions", JSONObject().put("receiverDeviceId", receiverDeviceId).put("providerDeviceId", hostId))
        SignalingSession(session.getString("id"), session.optString("providerPublicKey", ""), session.optString("relayUrl").ifBlank { null }, session.optLong("expiresAt", 0L) / 1000L)
    }
    ...
    override suspend fun denyRequest(requestId: String) = withContext(Dispatchers.IO) { request("POST", "/v1/sessions/$requestId/transition", JSONObject().put("state", "denied")); Unit }

    suspend fun getPendingProviderRequests(): List<ProviderRequest> = withContext(Dispatchers.IO) {
        val json = request("GET", "/v1/provider/requests"); val array = json.optJSONArray("requests") ?: JSONArray()
        buildList { for (index in 0 until array.length()) { val item = array.optJSONObject(index) ?: continue; add(ProviderRequest(item.getString("id"), item.getString("receiverDeviceId"), item.optString("state", "requested"), item.optLong("expiresAt", 0L))) } }
    }

    suspend fun markPresence() = withContext(Dispatchers.IO) { request("POST", "/v1/devices/presence", JSONObject()) }
    ...
    private fun request(method: String, path: String, body: JSONObject? = null, authenticated: Boolean = true): JSONObject {
        ...
        val connection = (URL(baseUrl.trimEnd('/') + path).openConnection() as HttpURLConnection).apply {
            ...
            if (authenticated) { val token = accessTokenProvider()?.takeIf { it.isNotBlank() } ?: throw LinkoNetworkException("device_auth_required"); setRequestProperty("Authorization", "Bearer $token") }
            if (body != null) { doOutput = true; setRequestProperty("Content-Type", "application/json") }
        }
        try { body?.let { payload -> connection.outputStream.use { it.write(payload.toString().toByteArray(Charsets.UTF_8)) } }; val status = connection.responseCode; val stream = if (status in 200..299) connection.inputStream else connection.errorStream; val text = stream?.bufferedReader()?.use { it.readText() }.orEmpty(); val json = if (text.isBlank()) JSONObject() else JSONObject(text); if (status !in 200..299) throw LinkoNetworkException(json.optString("error", "http_$status"), status); return json } finally { connection.disconnect() }
    }
}

data class ProviderRequest(val id: String, val receiverDeviceId: String, val state: String, val expiresAt: Long) { fun toIncomingRequest() = IncomingRequest(id, "LINKO friend", "L", receiverDeviceId, "REMOTE", "NOW") }
