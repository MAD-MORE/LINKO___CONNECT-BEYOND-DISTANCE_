...
class LinkoControlPlaneApi(
    ...
    suspend fun searchUsers(query: String): List<Friend> = withContext(Dispatchers.IO) {
        ...
        val json = request("GET", "/search?q=" + java.net.URLEncoder.encode(clean, "UTF-8"))
    ...
    override suspend fun denyRequest(requestId: String) = withContext(Dispatchers.IO) {
        request("POST", "/v1/sessions/$requestId/transition", JSONObject().put("state", "denied")); Unit
    }
    ...
    private fun request(method: String, path: String, body: JSONObject? = null, authenticated: Boolean = true): JSONObject {
        ...
        val connection = (URL(baseUrl.trimEnd('/') + path).openConnection() as HttpURLConnection).apply {
            ...
            if (authenticated) setRequestProperty("Authorization", "Bearer " + (accessTokenProvider()?.takeIf { it.isNotBlank() } ?: throw LinkoNetworkException("device_auth_required")))
