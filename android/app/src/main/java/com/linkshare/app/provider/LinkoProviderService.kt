...
class LinkoProviderService : Service() {
    ...
    private suspend fun heartbeat() {
        while (scope.isActive) {
            try { if (auth.hasRegisteredDevice()) api.markPresence() } catch (_: Exception) { }
            delay(15_000L)
        }
    }

    private fun accept(requestId: String) { scope.launch { runCatching { api.transition(requestId, "approved"); startApproved(requestId) }.onFailure { stopRunner(requestId); notificationManager().notify(NOTIFICATION_ID, serviceNotification("Connection failed", "The secure connection could not be started")) } } }
    private fun startApproved(requestId: String) { scope.launch { runCatching { api.transition(requestId, "signaling"); establishEngine(requestId) }.onFailure { stopRunner(requestId); notificationManager().notify(NOTIFICATION_ID, serviceNotification("Connection failed", "The secure connection could not be started")) } } }

    private suspend fun establishEngine(sessionId: String) {
        var lastError: Throwable? = null
        repeat(TUNNEL_CONFIG_RETRIES) { attempt ->
            try {
                val config = api.tunnelConfig(sessionId); val endpoint = config.getJSONObject("endpoint"); val host = endpoint.getString("host"); val port = endpoint.getInt("port"); val key = Base64.getUrlDecoder().decode(config.getString("key")); require(key.size == SESSION_KEY_SIZE) { "invalid_session_key" }
                stopRunner(sessionId)
                val runner = ProviderTunnelRunner(socket = ProviderSocketFactory.openDatagramSocket(), endpoint = InetSocketAddress(host, port), sessionId = sessionId, sessionKey = key, scope = scope)
                runners[sessionId] = runner; runner.start(); api.transition(sessionId, "connected"); notificationManager().notify(NOTIFICATION_ID, serviceNotification("Connected", "LINKO is sharing your connection securely")); return
            } catch (error: Exception) { lastError = error; if (attempt + 1 < TUNNEL_CONFIG_RETRIES) delay(TUNNEL_CONFIG_RETRY_MS) }
        }
        throw lastError ?: IllegalStateException("tunnel_start_failed")
    }

    private fun decline(requestId: String) { scope.launch { runCatching { api.transition(requestId, "denied") }; notificationManager().notify(NOTIFICATION_ID, serviceNotification("Provider Ready", "The request was declined")) } }
    ...
    companion object {
        const val ACTION_ACCEPT = "com.linkshare.app.provider.ACCEPT"; const val ACTION_DECLINE = "com.linkshare.app.provider.DECLINE"; const val ACTION_START_APPROVED = "com.linkshare.app.provider.START_APPROVED"; const val EXTRA_REQUEST_ID = "request_id"; private const val CHANNEL_ID = "linko_provider"; private const val NOTIFICATION_ID = 7001; private const val TUNNEL_CONFIG_RETRIES = 5; private const val TUNNEL_CONFIG_RETRY_MS = 1_000L; private const val SESSION_KEY_SIZE = 32
