...
import com.linkshare.app.network.LinkoIceCoordinator
...
class LinkoProviderService : Service() {
    ...
    override fun onCreate() {
        super.onCreate(); auth = LinkoAuth(this); api = LinkoDeviceControlApi(this, auth)
        acquireLocks(); createChannel(); startForeground(NOTIFICATION_ID, serviceNotification("Provider Ready", "LINKO is active and ready for connection requests"))
        registerNetworkCallback(); scope.launch { runCatching { api.ensureRegistered() } }; scope.launch { listenToRealtime() }; scope.launch { heartbeat() }
    }

    private fun acquireLocks() { runCatching {
        val pm = getSystemService(Context.POWER_SERVICE) as? PowerManager
        wakeLock = pm?.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "LINKO:ProviderWakeLock")?.apply { setReferenceCounted(false); acquire(24 * 60 * 60 * 1000L) }
        val wm = applicationContext.getSystemService(Context.WIFI_SERVICE) as? WifiManager
        wifiLock = wm?.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "LINKO:ProviderWifiLock")?.apply { setReferenceCounted(false); acquire() }
    ...
    private fun registerNetworkCallback() {
        ...
        networkCallback = object : ConnectivityManager.NetworkCallback() {
            override fun onAvailable(network: Network) { Log.i(TAG, "Validated provider network available; restoring approved sessions"); notificationManager().notify(NOTIFICATION_ID, serviceNotification("Provider Sharing Active", "Internet connection is active and stable")); scope.launch { delay(750); approvedRequests.toList().forEach { startApproved(it) } } }
            override fun onLost(network: Network) { Log.w(TAG, "Provider network lost; stopping data-plane sockets"); runners.values.toList().forEach { it.stop() }; runners.clear(); notificationManager().notify(NOTIFICATION_ID, serviceNotification("Network Interrupted", "LINKO will reconnect when internet returns…")) }
        }
        runCatching { cm.registerNetworkCallback(request, networkCallback!!) }
    }

    private fun hasActiveInternet(): Boolean { val cm = getSystemService(Context.CONNECTIVITY_SERVICE) as? ConnectivityManager ?: return false; val active = cm.activeNetwork ?: return false; val caps = cm.getNetworkCapabilities(active) ?: return false; return caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET) && caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED) }

    private suspend fun listenToRealtime() { LinkoRealtimeManager.events.collect { event -> if (!currentCoroutineContext().isActive) return@collect; when (event) { is LinkoRealtimeEvent.SessionStateChanged -> if (event.state == "requested") notifyPendingConnectionRequests(); else -> Unit } } }
    private suspend fun notifyPendingConnectionRequests() { runCatching { api.pendingProviderRequests() }.getOrDefault(emptyList()).forEach { request -> postRequestNotification(request.id, request.receiverDeviceId) } }
    private suspend fun heartbeat() { while (scope.isActive) { runCatching { api.ensureRegistered(); api.touchPresence() }; delay(15_000L) } }

    private fun accept(requestId: String) { scope.launch { if (!hasActiveInternet()) { notificationManager().notify(NOTIFICATION_ID, serviceNotification("Cannot Share", "No validated internet connection on this device")); return@launch }; runCatching { api.transition(requestId, "approved"); approvedRequests.add(requestId); startApproved(requestId) }.onFailure { stopRunner(requestId) } } }
    ...
    private fun startApproved(requestId: String) {
        scope.launch {
            ...
            runCatching {
                val accessToken = auth.currentAccessToken()
                if (!accessToken.isNullOrBlank()) LinkoIceCoordinator(LinkoSignalingClient(accessToken = accessToken)).publishLocalCandidate(requestId, socket)
            }.onFailure { Log.w(TAG, "Direct ICE candidate discovery unavailable: ${it.message}") }
            ...
            runners[requestId] = runner; runner.start(); notificationManager().notify(NOTIFICATION_ID, serviceNotification("Sharing Active", "Encrypted LINKO tunnel is live and routing traffic"))
        }
    }
    ...
    override fun onDestroy() { runCatching { networkCallback?.let { (getSystemService(Context.CONNECTIVITY_SERVICE) as? ConnectivityManager)?.unregisterNetworkCallback(it) } }; releaseLocks(); runners.values.toList().forEach { it.stop() }; runners.clear(); scope.cancel(); super.onDestroy() }

    companion object { private const val TAG = "LINKO_PROVIDER_SERVICE"; private const val NOTIFICATION_ID = 4101; private const val CHANNEL_ID = "linko_provider"; const val ACTION_ACCEPT = "com.linkshare.app.provider.ACCEPT"; const val ACTION_DECLINE = "com.linkshare.app.provider.DECLINE"; const val ACTION_START_APPROVED = "com.linkshare.app.provider.START_APPROVED"; const val EXTRA_REQUEST_ID = "linko.request.id" }
}