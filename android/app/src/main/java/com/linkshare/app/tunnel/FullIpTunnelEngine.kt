...
class HevFullIpTunnelEngine : FullIpTunnelEngine {
    ...
    override fun start(tunFd: FileDescriptor, socksHost: String, socksPort: Int) {
        ...
        val config = invoke(builder, "build")
    ...
    override fun isRunning(): Boolean {
        val instance = tunnel ?: return false
        return runCatching { invoke(instance, "isRunning") as Boolean }.getOrDefault(true)
    }

    override fun close() {
        val instance = tunnel ?: return
        runCatching { invoke(instance, "stop") }
        tunnel = null
    }

    private fun loadFirst(vararg names: String): Class<*>? =
    ...
    private fun invokeIfPresent(target: Any, name: String, vararg args: Any?) {
        runCatching { invoke(target, name, *args) }
    }
