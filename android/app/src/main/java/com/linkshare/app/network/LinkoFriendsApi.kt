...
class LinkoFriendsApi(private val accessTokenProvider: () -> String?) {
    ...
    suspend fun ensureProfile(displayName: String?, username: String? = null): JSONObject = withContext(Dispatchers.IO) {
        ...
        runCatching { callRpc("linko_get_or_create_profile", JSONObject().put("p_display_name", name)) }.getOrElse {
            post("/profile", JSONObject().apply { put("displayName", name); username?.trim()?.takeIf { it.isNotBlank() }?.let { put("username", it) } })
        }
    }

    suspend fun profile(): JSONObject = withContext(Dispatchers.IO) { runCatching { callRpc("linko_get_or_create_profile", JSONObject()) }.getOrElse { get("/profile") } }
    ...
    suspend fun search(query: String): List<FriendSearchResult> = withContext(Dispatchers.IO) {
        ...
        runCatching {
            ...
            buildList { for (i in 0 until array.length()) array.optJSONObject(i)?.let { add(parseFriend(it)) } }
        }.getOrElse {
    ...
    suspend fun friends(): JSONObject = withContext(Dispatchers.IO) {
        ...
        // The dashboard must not trust an old profile snapshot for online/offline state.
        // Resolve each friend's current provider heartbeat from the same authenticated
        // Supabase control plane used by LinkoEngineBridge before returning the list.
        ...
        for (i in 0 until array.length()) {
            ...
            runCatching { providerPresence(userId) }.onSuccess { presence ->
                friend.put("is_online", presence.first)
                friend.put("last_seen_at", presence.second)
                if (presence.third.isNotBlank()) friend.put("device_id", presence.third)
            }
        }
        result.put("friends", array)
    }

    private fun providerPresence(friendUserId: String): Triple<Boolean, Long, String> {
        val raw = callRpcRaw("linko_provider_for_user", JSONObject().put("p_friend_user_id", friendUserId))
        val json = JSONObject(raw)
        val device = json.optJSONObject("device")
        if (device == null) throw LinkoNetworkException("provider_not_available")
        return Triple(json.optBoolean("online", false), json.optLong("lastSeenAt", 0L), device.optString("id"))
    }
    ...
    suspend fun sendRequest(receiverUserId: String): JSONObject = withContext(Dispatchers.IO) {
        ...
        runCatching { callRpc("linko_send_friend_request", JSONObject().put("p_receiver_user_id", clean)) }.getOrElse { post("/requests", JSONObject().put("receiverUserId", clean)) }
    }
    suspend fun respond(requestId: String, accepted: Boolean): JSONObject = withContext(Dispatchers.IO) {
        ...
        runCatching { callRpc("linko_respond_friend_request", JSONObject().put("p_request_id", requestId).put("p_status", status)) }.getOrElse { post("/requests/respond", JSONObject().put("requestId", requestId).put("status", status)) }
    }
    ...
    private fun parseFriend(item: JSONObject): FriendSearchResult = FriendSearchResult(
        userId = item.optString("user_id"), linkoId = item.optString("linko_id"),
        ...
        isOnline = item.optBoolean("is_online", item.optBoolean("is_sharing", false)),
    ...
    private fun callRpc(functionName: String, body: JSONObject): JSONObject {
        ...
        return when { raw.startsWith("{") -> JSONObject(raw); raw.startsWith("[") -> JSONObject().put("results", JSONArray(raw)); else -> JSONObject() }
    }
    private fun callRpcRaw(functionName: String, body: JSONObject): String {
        ...
        val connection = (URL("$rpcBaseUrl/$functionName").openConnection() as HttpURLConnection).apply {
            requestMethod = "POST"; connectTimeout = 10_000; readTimeout = 15_000; doOutput = true
            setRequestProperty("apikey", BuildConfig.LINKO_SUPABASE_PUBLISHABLE_KEY); setRequestProperty("Authorization", "Bearer $token"); setRequestProperty("Accept", "application/json"); setRequestProperty("Content-Type", "application/json")
        }
        try {
            ...
            if (status !in 200..299) throw LinkoNetworkException(runCatching { JSONObject(text).optString("message", "http_$status") }.getOrDefault("http_$status"), status)
            return text.ifBlank { "{}" }
    ...
    private fun requestHttp(method: String, fullUrl: String, body: JSONObject?): JSONObject {
        ...
        val connection = (URL(fullUrl).openConnection() as HttpURLConnection).apply {
            requestMethod = method; connectTimeout = 10_000; readTimeout = 15_000; doOutput = body != null
            setRequestProperty("apikey", BuildConfig.LINKO_SUPABASE_PUBLISHABLE_KEY); setRequestProperty("Authorization", "Bearer $token"); setRequestProperty("Accept", "application/json"); setRequestProperty("Content-Type", "application/json")
        }
        try {
