...
val onboardingScreens = setOf(
    Screen.Welcome.route, Screen.CreateAccount.route, Screen.Verify.route,
    Screen.Profile.route, Screen.RegisterDevice.route, Screen.Permissions.route,
)

val bottomNavScreens = setOf(
    Screen.HomeEngine.route, Screen.Friends.route,
    Screen.SessionHistory.route, Screen.Settings.route,
)

fun activeNavTab(route: String?): String = when {
    route == null -> "HOME"
    route.startsWith("home") || route.startsWith("rx") || route.startsWith("connected")
        || route.startsWith("network") || route.startsWith("usage")
        || route.startsWith("session_details") || route.startsWith("provider")
        || route.startsWith("connection") || route.startsWith("reconnect") -> "HOME"
    route.startsWith("friend") || route.startsWith("find") || route.startsWith("request")
        || route.startsWith("incoming") || route.startsWith("blocked") -> "FRIENDS"
    route == Screen.SessionHistory.route -> "HISTORY"
    route.startsWith("settings") || route.startsWith("privacy") || route.startsWith("data")
        || route.startsWith("device") || route.startsWith("security") || route.startsWith("key")
        || route.startsWith("delete") -> "SETTINGS"
    else -> "HOME"
}
