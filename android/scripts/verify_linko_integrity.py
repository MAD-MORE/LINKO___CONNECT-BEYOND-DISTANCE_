...
def contains_session_contract(source: str, semantic_path: str) -> bool:
    if "$sessionId" not in semantic_path: return semantic_path in source
...
def main() -> int:
    ...
    for path in REQUIRED_FILES:
        if not path.is_file(): errors.append(f"missing required file: {path.relative_to(REPO)}")
    if errors: return fail(errors)
    kotlin_files = sorted(KOTLIN.rglob("*.kt"))
    for token in FORBIDDEN_PRODUCTION_REFERENCES:
        for path in kotlin_files:
            if path.name == "MockLinkShareRepository.kt": continue
            if token in read(path): errors.append(f"forbidden mock reference '{token}' in {path.relative_to(REPO)}")
    friends_api = read(KOTLIN / "network" / "LinkoFriendsApi.kt")
    if FRIEND_EDGE_BASE not in friends_api: errors.append("friend API is not wired to the configured Supabase Edge Function")
    for path in FRIEND_PATHS:
        if path not in friends_api: errors.append(f"friend workflow contract missing Android path: {path}")
    friends_ui = read(KOTLIN / "ui" / "screens" / "FriendsScreens.kt")
    if "relationshipStatus" not in friends_api: errors.append("FriendSearchResult.relationshipStatus missing")
    if "relationship_status" not in friends_api: errors.append("Android friend parser does not read relationship_status")
    if "relationshipStatus" not in friends_ui: errors.append("friend-search UI is not wired to relationshipStatus")
    ...
    for path, source in control_sources.items():
        if not contains_session_contract(source, path): errors.append(f"control-plane contract missing Android path/token: {path}")
    manifest = read(ANDROID_SRC / "AndroidManifest.xml")
    if "LinkShareVpnService" not in manifest: errors.append("VPN service is not declared in AndroidManifest.xml")
    if "android.permission.INTERNET" not in manifest: errors.append("INTERNET permission missing")
    ...
    if "LinkoEngineBridge" not in runtime: errors.append("LinkoRuntime does not reference LinkoEngineBridge")
    if "LinkoRuntime" not in app and "LinkoRuntime" not in viewmodel: errors.append("frontend does not reference LinkoRuntime")
    if not re.search(r"connect|request|session", engine_bridge, re.IGNORECASE): errors.append("LinkoEngineBridge contains no connection/session wiring")
    ...
    if "LINKO_CONTROL_PLANE_URL" not in gradle: errors.append("LINKO_CONTROL_PLANE_URL is not wired into BuildConfig")
    return fail(errors)
