...
# These are semantic contract fragments rather than literal URL strings. The
# Android client builds session-scoped signaling/tunnel paths dynamically.
CONTROL_PLANE_PATHS = (
    "/v1/devices/register",
    "/v1/devices/presence",
    "/v1/sessions",
    "/v1/sessions/$sessionId/signaling",
    "/v1/sessions/$sessionId/signaling/ticket",
    "/v1/sessions/$sessionId/tunnel",
)
...
def main() -> int:
    ...
    for path in CONTROL_PLANE_PATHS:
        source = signaling_client if "signaling" in path else control_api
