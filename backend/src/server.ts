...
async function supabaseRequest(path: string, method: string, key: string, payload?: Record<string, unknown>) { const response = await fetch(`${supabaseUrl}${path}`, { method, headers: { apikey: key, Authorization: `Bearer ${key}`, Accept: "application/json", ...(payload ? { "Content-Type": "application/json" } : {}) }, ...(payload ? { body: JSON.stringify(payload) } : {}) }); const text = await response.text(); let data: Record<string, unknown> = {}; try { data = JSON.parse(text) as Record<string, unknown>; } catch { /* handled by caller */ } return { response, data }; }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/v1/auth/signup") {
      ...
      const input = await body(req); if (typeof input.email !== "string" || typeof input.password !== "string") return json(res, 400, { error: "invalid_signup", requestId }, requestId); if (input.password.length < 6 || input.password.length > 72) return json(res, 400, { error: "password_length_invalid", requestId }, requestId);
      const email = input.email.trim().toLowerCase(); const signup = await supabaseRequest("/auth/v1/signup", "POST", supabasePublishableKey, { email, password: input.password, data: input.data ?? {} });
      ...
      const user = signup.data.user as { id?: string; email?: string } | undefined; if (!user?.id) return json(res, 502, { error: "signup_user_missing", requestId }, requestId);
      const confirm = await supabaseRequest(`/auth/v1/admin/users/${encodeURIComponent(user.id)}`, "PUT", supabaseSecretKey, { email_confirm: true }); if (!confirm.response.ok) return json(res, 502, { error: "signup_confirmation_failed", requestId }, requestId);
      const login = await supabaseRequest("/auth/v1/token?grant_type=password", "POST", supabasePublishableKey, { email, password: input.password }); if (!login.response.ok) return json(res, 502, { error: "signup_session_failed", requestId }, requestId);
    ...
    if (req.method === "GET" && providerUserMatch && providerUserMatch[1]) {
      const friendUserId = decodeURIComponent(providerUserMatch[1]);
      if (!(await areUsersFriends(authenticatedDevice.userId, friendUserId))) return json(res, 403, { error: "friendship_required", requestId }, requestId);
      const provider = postgresStore ? await postgresStore.getProviderDeviceForUser(friendUserId) : await store.getProviderDeviceForUser(friendUserId);
      if (!provider || provider.revokedAt) return json(res, 404, { error: "provider_not_available", requestId }, requestId);
      const freshnessMs = Date.now() - provider.lastSeenAt;
      if (freshnessMs > 90_000) return json(res, 409, { error: "provider_offline", requestId }, requestId);
      return json(res, 200, { device: provider, online: true, lastSeenAt: provider.lastSeenAt }, requestId);
    }
    ...
    if (req.method === "POST" && url.pathname === "/v1/sessions") { const input = await body(req); if (typeof input.receiverDeviceId !== "string" || typeof input.providerDeviceId !== "string") return json(res, 400, { error: "invalid_session_request", requestId }, requestId); if (authenticatedDevice.id !== input.receiverDeviceId && authenticatedDevice.id !== input.providerDeviceId) return json(res, 403, { error: "session_party_required", requestId }, requestId); const receiver = await store.getDevice(input.receiverDeviceId); const provider = await store.getDevice(input.providerDeviceId); if (!receiver || !provider) return json(res, 404, { error: "device_not_found", requestId }, requestId); if (!(await areUsersFriends(receiver.userId, provider.userId))) return json(res, 403, { error: "friendship_required", requestId }, requestId); const session = await store.createSession(input.receiverDeviceId, input.providerDeviceId); const key = createSessionTunnelKey(session.id); if (tunnelEndpoint) tunnelEndpoint.addSession(session.id, key); return json(res, 201, session, requestId); }
    const transitionMatch = url.pathname.match(/^\/v1\/sessions\/([^/]+)\/transition$/);
    if (req.method === "POST" && transitionMatch && transitionMatch[1]) { const session = await store.getSession(transitionMatch[1]); if (!session) return json(res, 404, { error: "session_not_found", requestId }, requestId); if (session.receiverDeviceId !== authenticatedDevice.id && session.providerDeviceId !== authenticatedDevice.id) return json(res, 403, { error: "session_party_required", requestId }, requestId); const input = await body(req); if (typeof input.state !== "string") return json(res, 400, { error: "state_required", requestId }, requestId); const next = input.state as SessionState; if (next === "approved" && session.providerDeviceId !== authenticatedDevice.id) return json(res, 403, { error: "provider_approval_required", requestId }, requestId); const updated = await store.transitionSession(session.id, next); if (["revoked", "expired", "denied"].includes(next)) { revokeSessionTunnelKey(session.id); tunnelEndpoint?.removeSession(session.id); } return json(res, 200, updated, requestId); }
